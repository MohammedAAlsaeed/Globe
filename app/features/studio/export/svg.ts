    ...
    .map((item, i) => {
      ...
      const marks = p.marks
        ? [
            ...
            .join("")
        : "";
      return `<defs><clipPath id="tile-${i}"><rect x="${item.x}" y="${item.y}" width="${item.width}" height="${item.height}"/></clipPath></defs><g clip-path="url(#tile-${i})"><g transform="${transform}">${goreMarkup(s, item.gore, assets[item.gore]?.url ?? "", p.outline)}</g></g>${marks}`;
    })
    .join("");
